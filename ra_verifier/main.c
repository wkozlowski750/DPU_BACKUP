...
static void lcore_main(struct lcore_args *function_args) {
	...
	while(!quit) {
		...
		int swift_pkts = filter(burst_rx, burst_rx_filtered, ctr, nonce, nb_rx, sigs, mask_ptr, ret_ptr);
		...
		result = sha_perform_bulk(&resources, doca_dst_buffers, burst_rx_filtered, swift_pkts, sha_partial_hash_task);
		// for (uint32_t i = 0; i < BURST_SIZE; i++) printf("original src data %p, src_data %p\n", original_src_buf[i], resources.src_doca_buf[i]);
		if (result != DOCA_SUCCESS) {
		...
		for (int i = 0; i < swift_pkts; i++) {
			...
			if (!match) {
				// DOCA_LOG_INFO("Core %u: Signature did not match. Expected: %lx%lx%lx%lx Recieved: %lx%lx%lx%lx\n", core_id,
				// 				sigs[i * 4], sigs[i * 4 + 1], sigs[i * 4 + 2], sigs[i * 4 + 3], data[0], data[1], data[2], data[3]);
				continue;
			}
			ver_ctr++;
