...
void verify(struct rte_mbuf **m, uint16_t *ctr, uint64_t *nonce, uint16_t quant, uint64_t *key, uint64_t *mask, int *ret, uint32_t *ver_ctr) {
	...
	for (uint16_t i = 0; i < quant; i++) {
		...
		retval = rte_hash_lookup(prover_keys, hdr->src_addr.addr_bytes);
		free(tmp);
		...
		struct prover_props *prover  = provers[retval];
        ...
        *p = OSSL_PARAM_construct_end();
		...
		uint64_t *out_long = (uint64_t *)out;
...
int filter(struct rte_mbuf **m, struct rte_mbuf **m_dup, uint16_t *ctr, uint64_t *nonce, uint16_t quant, uint64_t *sigs, uint64_t *mask, int *ret) {
	struct rte_ether_hdr *hdr;
	// struct rte_vlan_hdr *vlan_hdr;
	uint32_t core_id = rte_lcore_id();
	// struct verif_ret ret;
	...
	for (uint16_t i = 0; i < quant; i++) {
		...
		// printf("Core: %u state matched\n", core_id);

		memcpy(&sigs[order_ctr * 4], &buf[SWIFT_MSG_LEN], SWIFT_HASH_LEN);
		// printf("Core %u: Copied sig\n", core_id);
		m_dup[order_ctr] = m[i];
		// printf("Core %u: Copied Swift pkt\n", core_id);
		pkts_processed |= 1 << i;
		order_ctr++;
	}
	free(buf);
