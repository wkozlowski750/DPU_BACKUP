...
void cmd_attest_parsed(void *parsed_result, struct cmdline *cl, __rte_unused void *data) {
	...
	*ctr++;
	...
	RTE_ETH_FOREACH_DEV(port) {
		for (uint8_t test_number = 0; test_number < 10; test_number++) {
			...
			// if (nb_rx_tot == 0) continue

			for (uint32_t i = 0; i < prover_quant + 1; i++) {
				...
				char *tmp;
				sprintf(tmp, "cpu_ra_test_%u.csv", prover_quant);
				append_time_to_csv(tmp, time_diff);
...
void append_time_to_csv(const char* filename, long long time_ns) {
    ...
    fprintf(file, "%lld\n", time_ns); // Write the time in nanoseconds
    fclose(file);
}